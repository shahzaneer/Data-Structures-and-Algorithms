#include <iostream>
using namespace std;

// ! Implementation of Priority Queue
// As per the policy : We have to serve the aged person first and for two persons with equal age we will do
// FIFO operation as Queue is supposed to do so!
// Dequeue Operation will remain the same!

struct PizzaQueue
{
    int rear = -1;
    int front = -1;
    int size;
    string *orderedPizza;
    int *orderNumber;
    int *age;
};

PizzaQueue *q = NULL; // making of a global pointer

void prioritize();

int isFull()
{
    if (((q->rear + 1) % q->size) == q->front)
    {
        return 1;
    }
    return 0;
}

int isEmpty()
{
    if (q->rear == -1 && q->front == -1)
    {
        return 1;
    }
    return 0;
}

void placeOrder(int age, string value, int orderNo)
{
    if (isFull() == 1)
    {
        cout << "We cannot place your Order! Kindly wait for the previous orders to be completed ! " << endl;
        return;
    }

    // We have to change this method and enqueue the orders with respect to the age priority 

    else if (isEmpty() == 1)
    {
        q->rear = q->front = 0;
        q->orderedPizza[q->rear] = value;
        q->orderNumber[q->rear] = orderNo;
        q->age[q->rear] = age;

        cout << "Your Order is placed with ID " << q->orderNumber[q->rear] << endl;
        cout << "The Ordered item is " << q->orderedPizza[q->rear] << endl;
    }

    else
    {
        q->rear = (q->rear + 1) % q->size;
        q->orderedPizza[q->rear] = value;
        q->orderNumber[q->rear] = orderNo;
        q->age[q->rear] = age;
        cout << "Your Order is placed with ID " << q->orderNumber[q->rear] << endl;
        cout << "The Ordered item is " << q->orderedPizza[q->rear] << endl;
        prioritize();
    }
}

void orderDequeue()
{
    if (isEmpty() == 1)
    {
        cout << "There is no order in the Queue " << endl;
        return;
    }

    else if (q->rear == q->front)
    {
        cout << "The order completed is ID Number " << q->orderNumber[q->front] << endl;
        cout << "The order Completed is  " << q->orderedPizza[q->front] << endl;
        q->rear = q->front = -1;
    }
    else
    {
        cout << "The order completed is ID Number " << q->orderNumber[q->front] << endl;
        cout << "The order Completed is " << q->orderedPizza[q->front] << endl;
        q->front = (q->front + 1) % q->size;
    }
}

void display()
{
    if (isEmpty() == 1)
    {
        cout << "There is no order in the queue!" << endl;
        return;
    }
    else
    {
        //  in case of printing
        int i = q->front;
        while (i != q->rear)
        {
            // cout << "Pakistan Noor hai aur noor ko zawaal nhi!" << endl;

            if (q->orderedPizza[i] != "")

            {
                // cout << "Pakistan Noor hai aur noor ko zawaal nhi!" << endl;
                cout << "ID Number : " << q->orderNumber[i] << endl;
                cout << "Ordered Item :  " << q->orderedPizza[i] << endl;
            }
            i = (i + 1) % q->size;
        }

        cout << "ID Number : " << q->orderNumber[q->rear] << endl;
        cout << "Ordered Item :  " << q->orderedPizza[q->rear] << endl;
    }
}

void CurrentOrder()
{
    if (isEmpty() == 1)
    {
        cout << "There is no Order in the queue !" << endl;
        return;
    }

    // front element
    cout << "The Current Order ID is " << q->orderNumber[q->front] << endl;
    cout << "The Current Order is " << q->orderedPizza[q->front] << endl;
}

void latestOrder()
{
    if (isEmpty() == 1)
    {
        cout << "There is no Order in the Queue " << endl;
        return;
    }

    // rear element
    cout << "The Current Order ID is " << q->orderNumber[q->rear] << endl;
    cout << "The Latest order is " << q->orderedPizza[q->rear] << endl;
}

void prioritize()
{
    string tempStr;
    int tempID;
    int tempAge;

    for (int i = 0; i < q->size; i++)
    {
        for (int j = 0; j < q->size; j++)
        {
            if (q->age[i] > q->age[j])
            {

                // Swapping ID number
                tempID = q->age[i];
                q->age[i] = q->age[j];
                q->age[j] = tempID;

                // Swapping order
                tempStr = q->orderedPizza[i];
                q->orderedPizza[i] = q->orderedPizza[j];
                q->orderedPizza[j] = tempStr;

                // Swapping ages
                tempAge = q->age[i];
                q->age[i] = q->age[j];
                q->age[j] = tempAge;
            }
        }
    }
}
int main()
{
    q = new PizzaQueue; // DMA things
    int size;
    cout << "State the number of Orders \n";
    cin >> size;
    q->size = size;
    q->orderedPizza = new string[q->size];
    q->orderNumber = new int[q->size];
    q->age = new int[q->size];

    for (int i = 0; i < size; i++)
    {
        q->orderedPizza[i] = "";
    }
    int orderID = 0;
    int option;

    do
    {
        cout << "--------------------------------------------------\n";
        cout << "Enter options to perform corresponding actions \n";

        cout << "1 -  Place Order\n"
             << "2 -  Complete Current Order\n"
             << "3 -  Display Order Waiting list \n"
             << "4 -  Current Order\n"
             << "5-   Latest order\n"
             << "-1 - Exit\n";
        cout << "--------------------------------------------------\n";

        cin >> option;
        switch (option)
        {
        case 1:
        {
            int age;
            cout << "Enter your Age \n";
            cin >> age;
            int val;
            cout << "Menu : \n"
                 << "0) Chicken Fajita \n"
                 << "1) Chicken Tikka \n"
                 << "2) Cheese Lover \n"
                 << "3) Barbeque Special \n"
                 << "4) Seekh Kabab Pizza \n"
                 << "5) Super Italian \n ";

            cout << "Enter value \n";
            string pizzaList[6] = {"Chicken Fajita", "Chicken Tikka", "Cheese Lover", "Barbeque Special", "Seekh Kabab Pizza", "Super Italian"};

            cin >> val;
            if (val >= 0 && val <= 5)
            {
                // cout << "Pakistan Noor hai !"

                placeOrder(age, pizzaList[val], ++orderID);
                break;
            }
            else
            {
                continue;
            }
        }

        case 2:
        {
            orderDequeue();
            break;
        }

        case 3:
        {
            display();
            break;
        }
        case 4:
        {
            CurrentOrder();
            break;
        }
        case 5:
        {
            latestOrder();
            break;
        }
        case -1:
        {
            cout << "exiting . . . \n";
            break;
        }
        }
    } while (option != -1);

    return 0;
}
